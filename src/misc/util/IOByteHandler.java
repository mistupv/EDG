...
public class IOByteHandler
{
	...
	public long readLong(final int byteIndex, final int bitIndex, final ByteOrder byteOrder)
	{
		...
		if (bitIndex == 0)
			return (long) (byteOrder == ByteOrder.BIG_ENDIAN ? a << 56 | b << 48 | c << 40 | d << 32 | e << 24 | f << 16 | g << 8 | h
															 : h << 56 | g << 48 | f << 40 | e << 32 | d << 24 | c << 16 | b << 8 | a);
		else
		{
			int i = byteArray[byteIndex + 8];

			return (long) (byteOrder == ByteOrder.BIG_ENDIAN ? (a & 0xFF << bitIndex) << 56 | b << 48 + bitIndex | c << 40 + bitIndex | d << 32 + bitIndex | e << 24 + bitIndex | f << 16 + bitIndex | g << 8 + bitIndex | h << bitIndex | i & 0xFF >>> 8 - bitIndex
