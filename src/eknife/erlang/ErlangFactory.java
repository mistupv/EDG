...
public class ErlangFactory
{
	...
	public static File createErlangFile(String outputPath, EDG edg)
	{
		return ErlangFactory.createErlangFile(outputPath, edg, null);
	}
	public static File createErlangFile(String outputPath, EDG edg, List<Node> slice)
	{
		final ErlangFactory erlangFactory = new ErlangFactory();
		final OtpErlangList program = erlangFactory.generate(edg, slice);
		Launcher.getLauncher().launch("saver", "save", program);

		final File tempFile = new File(ErlangFactory.config.getScriptsPath() + "tempSliced.erl");
		final File obtainedFile = new File(outputPath);
	...
	private ErlangFactory()
	{
		
	}

	private OtpErlangList generate(EDG edg, List<Node> slice)
	{
		this.slice = slice;
	...
	private void parseEDG(List<OtpErlangObject> bodyList, EDG edg)
	{
		...
		// Others
		final OtpErlangList others = (OtpErlangList) root.getData().getInfo()[0];
		...
		{
			if (this.slice != null && !this.slice.contains(function))
				continue;
	...
	{
		final String functionName = function.getData().getText();
		final long functionArity = (int) function.getData().getInfo()[0];
	...
	private OtpErlangTuple parseFunctionExpression(Node function)
	{
		...
		switch (functionType)
		{
			...
			case AnonymousFunction:
				...
				break;
			default:
				throw new RuntimeException("Function type not contemplated: " + functionType);
		}
	...
	{
		final String functionName = function.getData().getText();
		final long functionArity = (long) function.getData().getInfo()[0];
		final OtpErlangObject[] functionElements = new OtpErlangObject[3];
	...
	private OtpErlangTuple parseClause(Node clause)
	{
		...
		clauseElements[3] = (OtpErlangObject) guards.getData().getInfo()[0];
	...
	{
		if (this.slice != null && !this.slice.contains(pattern))
		...
		switch (patternType)
		{
			...
			default:
				throw new RuntimeException("Pattern type not contemplated: " + patternType);
		}
	}
	...
	private OtpErlangTuple parseUnaryOperationPattern(Node operation)
	{
		...
		final String operationText = operation.getData().getText();
		...
		operationElements[1] = new OtpErlangLong(1);
		operationElements[2] = new OtpErlangAtom(operationText);
	...
	private OtpErlangList parseExpressions(List<Node> expressions, boolean transformUnused)
	{
		...
		{
			if (!transformUnused && this.slice != null && !this.slice.contains(expression))
				continue;
	...
	{
		if (this.slice != null && !this.slice.contains(expression))
		...
		switch (expressionType)
		{
			...
			default:
				throw new RuntimeException("Instruction type not contemplated: " + expressionType);
		}
	}
	...
	{
		final String remoteText = remote.getData().getText();
		final int remoteDelimiterIndex = remoteText.indexOf(":");
		final String remoteClassId = remoteText.substring(0, remoteDelimiterIndex);
		final String remoteFunctionId = remoteText.substring(remoteDelimiterIndex + 1);
	...
	{
		final String nodeText = operation.getData().getText();
		final List<Node> operationExpressions = GraphTraverser.getChildren(operation, EdgeInfo.Type.NormalControl);
	...
	private OtpErlangTuple parseBinElementExpression(Node binElement)
	{
		...
		if (valueElement instanceof OtpErlangAtom && valueElement.toString().equals("undef"))
		{
			...
			{
				final int binElements = binElementExpression1.getData().getText().length();
				final OtpErlangObject[] operationElements = new OtpErlangObject[5];
	...
	{
		final String varName = var.getData().getText();
		...
		varElements[2] = new OtpErlangAtom(varName);

		return new OtpErlangTuple(varElements);
	}
	private OtpErlangTuple parseAtom(Node atom)
	{
		final String atomText = atom.getData().getText();
	...
	{
		final String stringText = string.getData().getText();
	...
	{
		final String nodeText = integer.getData().getText();
	...
	{
		final String nodeText = _char.getData().getText();
		final long value = Long.parseLong(nodeText);
