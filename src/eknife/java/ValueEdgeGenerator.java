...
public class ValueEdgeGenerator {
	...
	private void generateEqualityEdges()
	{
		...
		for (Node equality : equalities)
		{
			...
			final Node value = LASTTraverser.getChild(equality, 1);

			this.last.addEdge(equality, pattern, 0, new EdgeInfo(EdgeInfo.Type.Value));
			this.last.addEdge(value, equality, 0, new EdgeInfo(EdgeInfo.Type.Value));
		}
	}
	...
	private void generateDataConstructorAccessEdges()
	{
		final List<Node> dataConstructorAccesses = LASTTraverser
				.getNodes(this.last, NodeInfo.Type.DataConstructorAccess);
