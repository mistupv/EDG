...
import upv.slicing.edg.graph.VariableInfo.Context;
...
{	
	...
	private void collectInfo()
	{
		...
		while (!workList.isEmpty())
		{
			...
			final Function<ControlFlowTraverser.NodeWork<State>, Set<ControlFlowTraverser.NodeWork<State>>> newStates = new Function<ControlFlowTraverser.NodeWork<State>, Set<ControlFlowTraverser.NodeWork<State>>>() {
				public Set<ControlFlowTraverser.NodeWork<State>> apply(ControlFlowTraverser.NodeWork<State> nodeWork)
				{
					...
					final Set<ControlFlowTraverser.NodeWork<State>> newStates = new HashSet<ControlFlowTraverser.NodeWork<State>>();
					...
					for (State state : states)
					{
						...
						newStates.add(new ControlFlowTraverser.NodeWork<State>(newNode, new State(newNode, uses, definitions)));
					}
 ...
 */		
			...
			for (Node useNode : usesNodes)
			{
				...
				if (isGlobalVariable && definitionResultNode != useNode)
				{
					...
					{
						final List<Edge> calleeResults = EDGTraverser.getEdges(EDGTraverser.getParent(definitionNode), LASTTraverser.Direction.Backwards, EdgeInfo.Type.Input);
					...
					if (isDefinitionCall)
					{
						...
						final List<Edge> calledFunctionClauseEdges = EDGTraverser
								.getEdges(calleeDefCallResult, LASTTraverser.Direction.Forwards, EdgeInfo.Type.Input);
