...
{	
	...
	private Set<State> getVariableStates(Node node, boolean global)
	{
		...
		if (nodeInfo instanceof VariableInfo)
		{
			...
			if (!global || variableInfo.isGlobal())
			{
				...
				final Node grandParent = EDGTraverserNew.getParent(EDGTraverserNew.getParent(node));
				final VariableId variableId;
				if (grandParent.getData().getType() == NodeInfo.Type.DataConstructorAccess)
				{
					final Node index = EDGTraverserNew.getChild(EDGTraverserNew.getChild(grandParent, 1),0);
	...
	{
		final List<Node> workList = EDGTraverserNew.getNodes(this.edg, NodeInfo.Type.Clause);
		final Set<Node> worksDone = new HashSet<Node>();

		while (!workList.isEmpty())
		{
			final Node clause = workList.remove(0);
			final Node parametersNode = EDGTraverserNew.getChild(clause, NodeInfo.Type.ParameterIn);
			final Node clauseResult = EDGTraverserNew.getChild(clause, NodeInfo.Type.Result);
			final List<Node> callResults = EDGTraverserNew.getOutputs(clauseResult, EDGTraverserNew.Direction.Forwards);
			...
			final Function<NodeWork<State>, Set<NodeWork<State>>> newStates = new Function<NodeWork<State>, Set<NodeWork<State>>>() {
				public Set<NodeWork<State>> apply(NodeWork<State> nodeWork)
				{
					...
					final Set<State> states = FlowEdgeGeneratorNew.this.getNodeStates(newNode, true);
			...
			{
				final Node outputClause = EDGTraverserNew.getAncestor(callResult, NodeInfo.Type.Clause);
	...
	private void addEdges()
	{
		...
		{	
			...
			final boolean isDefinitionCall = !isDefinitionParameters && EDGTraverserNew.getSibling(definitionNode, 0).getData().getType() == NodeInfo.Type.Call;
...
// TODO ESTE TRATAMIENTO NO DEBE HACERSE AQUI, YA QUE ESTA CLASE ES COMUN A TODOS LOS SLICERS E INDEPENDIENTE DEL LENGUAJE
				final Node callNode = EDGTraverserNew.getSibling(definitionNode,0);
				final Node calleeNode = EDGTraverserNew.getChild(callNode,0);
				final Node nameNode = EDGTraverserNew.getChild(calleeNode, 1);
				final Node nameChild = EDGTraverserNew.getChild(nameNode, 0);
				final String Name = nameChild.getData().getType() == NodeInfo.Type.Expression ? EDGTraverserNew.getChild(nameChild,0).getData().getName() : "";
			...
			{
				final Node definitionResultNode0 = EDGTraverserNew.getResult(definitionNode); 
				final Node definitionResultNode1 = definitionResultNode0 == null ? definitionNode : definitionResultNode0;
				definitionResultNode = definitionResultNode1.getData().getType() == NodeInfo.Type.Expression ? EDGTraverserNew.getResult(definitionResultNode1) : definitionResultNode1;
			}
			...
			final String className0 = isDefinitionCall ? this.getScopeClass(EDGTraverserNew.getParent(definitionResultNode)) : definitionNode.getData().getInfo().getClassName();
	...
	continue;
			...
			if (declarationNode != null && declarationNode != definitionNode) 
				if (isDefinitionParameters)
					this.edg.addEdge(declarationNode, definitionNode, 0, new EdgeInfo(EdgeInfo.Type.Flow, letThroughConstraint));
				else if (isDefinitionVariable)
				{
					final Node definitionName = EDGTraverserNew.getSibling(definitionResultNode, 0);
 ...
 */		
			...
			for (Node useNode : usesNodes)
			{
				...
				final boolean isUseCall = !isUseLastNode && EDGTraverserNew.getSibling(useNode, 0).getData().getType() == NodeInfo.Type.Call;
				final boolean isUseVariable = !isUseLastNode && !isUseCall;
				final Node useNode1 = !isUseCall ? EDGTraverserNew.getResult(useNode) : getArgumentInNode(useNode);
				
				// INSIDE FUNCTIONS, GLOBALS & LOCALS
				if(isDefinitionVariable && isUseVariable)
				{
					final Node sibling = EDGTraverserNew.getSibling(useNode1, NodeInfo.Type.Value);
					if (sibling.getData().getType() == NodeInfo.Type.DataConstructorAccess) 
					{
						final Node variableIndex = variableId.getVariableIndex();
						final Node index = EDGTraverserNew.getChild(EDGTraverserNew.getChild(sibling, NodeInfo.Type.Variable),NodeInfo.Type.Value);
				...
				if (isGlobalVariable && definitionResultNode != useNode)
				{
					...
					{	// useNode1 is node argumentIn
						this.edg.addEdge(useNode1, EDGTraverserNew.getResult(useNode), 0, new EdgeInfo(EdgeInfo.Type.Flow, addConstraint));
					...
					{
						final List<Edge> calleeResults = EDGTraverserNew.getEdges(EDGTraverserNew.getParent(definitionNode), Direction.Backwards, EdgeInfo.Type.Input);
						for (Edge calleeResult : calleeResults)
						{
							final Node clauseArgsInNode = EDGTraverserNew.getSibling(EDGTraverserNew.getParent(calleeResult.getFrom()), NodeInfo.Type.ArgumentIn);
							this.edg.addEdge(clauseArgsInNode, definitionResultNode, 0, new EdgeInfo(EdgeInfo.Type.Input, letThroughConstraint));
						}
					...
					{
						final Node parameterOutNode = EDGTraverserNew.getSibling(useNode, NodeInfo.Type.ParameterOut); 
						this.edg.addEdge(definitionResultNode, parameterOutNode, 0, new EdgeInfo(EdgeInfo.Type.Flow, removeConstraint));
						final Node routineNode = EDGTraverserNew.getAncestor(definitionNode, NodeInfo.Type.Routine);
					...
					{
						final Node parametersNode = EDGTraverserNew.getSibling(useNode,NodeInfo.Type.ParameterOut); 
					...
					if (isDefinitionCall)
					{
						...
						final Node calleeDefCallResult = EDGTraverserNew.getResult(EDGTraverserNew.getSibling(definitionResultNode, 0));
						this.edg.addEdge(calleeDefCallResult, definitionResultNode, 0, new EdgeInfo(EdgeInfo.Type.Flow, removeConstraint));
						
						final List<Edge> calledFunctionClauseEdges = EDGTraverserNew.getEdges(calleeDefCallResult, Direction.Forwards, EdgeInfo.Type.Input);
						for (Edge clauseEdge: calledFunctionClauseEdges)
						{
							final Node clauseParameterOut = EDGTraverserNew.getChild(clauseEdge.getTo(), NodeInfo.Type.ParameterOut);
							this.edg.addEdge(clauseParameterOut, definitionResultNode, 0, new EdgeInfo(EdgeInfo.Type.Output, letThroughConstraint));
						}
...
// TODO Discutir si habria que coger esta definición cuando haya llamadas a esta funcion
					if (isDefinitionParameters)
					{
						final Node declarationResultNode = EDGTraverserNew.getResult(declarationNode);
	...
	private Node getDeclaration(VariableId variableId, String clazz, Node definitionNode)
	{
		final Predicate<Node> collectAndStop = new Predicate<Node>() {
			public boolean test(Node node)
			{
				...
				if (!variableInfo.isDeclaration())
					return false;
				final VariableId variableId0 = FlowEdgeGeneratorNew.this.getVariableId(node);
				if (!variableId.equals(variableId0))
					return false;
		...
		final Node moduleNode = EDGTraverserNew.getModuleByName(edg, clazz);
...
//		return variables.isEmpty() ? null : variables.get(0);
	...
	private List<Variable> getDefinitions()
	{
		...
		for (Node definitionNode : definitionNodes)
		{
			...
			{
				final Node dataAccessResultNode = EDGTraverserNew.getSibling(grandParent,1);
				final Node index = EDGTraverserNew.getChild(EDGTraverserNew.getChild(grandParent,1),0);
	...
	private Set<Node> getUses(VariableId variableId, Node definitionNode, Node lastNode)
	{
		final Predicate<Node> collect = new Predicate<Node>() {
			public boolean test(Node node)
			{
				...
				if (node == lastNode)
					return true;
				final Set<State> states = FlowEdgeGeneratorNew.this.getNodeStates(node, false);
				for (State state : states)
...
//	} 
	...
	{
		final Node clauseNode = EDGTraverserNew.getAncestor(node, NodeInfo.Type.Clause);
		if (clauseNode != null)
			return EDGTraverserNew.getChild(clauseNode, NodeInfo.Type.Result);
		return EDGTraverserNew.getResult(node);
	}
	private Node getArgumentsNode(Node node)
	{
		final Node callNode = EDGTraverserNew.getSibling(node, 0);
		return EDGTraverserNew.getChild(callNode, NodeInfo.Type.Arguments);
	}
	private Node getArgumentInNode(Node node)
	{
		final Node callNode = EDGTraverserNew.getSibling(node, 0);
		return EDGTraverserNew.getChild(callNode, NodeInfo.Type.ArgumentIn);
	}
	private Node getArgumentOutNode(Node node)
	{
		final Node callNode = EDGTraverserNew.getSibling(node, 0);
		return EDGTraverserNew.getChild(callNode, NodeInfo.Type.ArgumentOut);
	}
...
//	}
	...
	{
		final Node callNode = EDGTraverserNew.getSibling(callResult, 0);
		final Node scopeNode = EDGTraverserNew.getChild(EDGTraverserNew.getChild(callNode, 0),0);
		...
		else
		{
			...
			final Node moduleRef1 = moduleRef0.getData().getType() != NodeInfo.Type.Expression ? moduleRef0 : EDGTraverserNew.getChild(moduleRef0, 0);
			final Node moduleRef = moduleRef1.getData().getType() == NodeInfo.Type.TypeTransformation ? EDGTraverserNew.getChild(EDGTraverserNew.getChild(moduleRef1, 0),0) :
				moduleRef1;
