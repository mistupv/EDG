...
public class WorkList
{
	...
	{
// TODO Arreglar
Cronometro.empezar("1.4.3.1.1 - Done");
		final boolean done = this.isDone(work);
Cronometro.terminar("1.4.3.1.1 - Done");
if (done)
return true;
Cronometro.empezar("1.4.3.1.2 - Pending");
		final boolean pending = this.isPending(work);
Cronometro.terminar("1.4.3.1.2 - Pending");
	...
	private Map<Integer, Map<Integer, List<Work>>> getWorks(Type type)
	{
		switch (type)
		{
			...
			case Pending:
				return this.pending;
			default:
				throw new RuntimeException("Type not contempled yet: " + type);
		}
	}
